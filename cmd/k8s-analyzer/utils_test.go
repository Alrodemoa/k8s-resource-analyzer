...
func TestParseMemoryValue_MiSuffix(t *testing.T) {
	...
	if got != 512.0 {
		t.Errorf("got %.2f, want 512.0", got)
	}
}
...
func TestParseMemoryValue_CaseInsensitive(t *testing.T) {
	...
	if a != b {
		t.Errorf("case insensitive mismatch: %.2f != %.2f", a, b)
	}
}
...
func TestParseMemoryValue_NA(t *testing.T) {
	for _, v := range []string{"n/a", "N/A", "", "<none>", "Н/Д"} {
		...
		if got != 0 {
			t.Errorf("parseMemoryValue(%q) = %.2f, want 0", v, got)
		}
	}
}
...
func TestParseMemoryValue_Plain(t *testing.T) {
	...
	if got != 1024.0 {
		t.Errorf("got %.2f, want 1024.0", got)
	}
}
...
func TestParseCPUValue_Millicores(t *testing.T) {
	...
	if got != 500.0 {
		t.Errorf("got %.2f, want 500.0", got)
	}
}
...
func TestParseCPUValue_NA(t *testing.T) {
	for _, v := range []string{"n/a", "", "<none>", "Н/Д"} {
		got := parseCPUValue(v)
		if got != 0 {
			t.Errorf("parseCPUValue(%q) = %.2f, want 0", v, got)
		}
	}
}
...
func TestFormatCPUValue_RoundTrip(t *testing.T) {
	...
	if parsed != original {
		t.Errorf("round-trip failed: %.2f → %q → %.2f", original, formatted, parsed)
	}
}
...
func TestFormatMemoryValue_GiRange(t *testing.T) {
	got := formatMemoryValue(2048) // 2 GiB
	if got != "2.0Gi" {
		t.Errorf("got %q, want \"2.0Gi\"", got)
	}
}
...
func TestSanitizeSheetName_RemovesSpecialChars(t *testing.T) {
	...
	for input, want := range cases {
		...
		if got != want {
			t.Errorf("sanitizeSheetName(%q) = %q, want %q", input, got, want)
		}
	}
}

func TestSanitizeSheetName_TruncatesTo31(t *testing.T) {
	long := "abcdefghijklmnopqrstuvwxyz123456" // 32 chars
	got := sanitizeSheetName(long)
	if len(got) > 31 {
		t.Errorf("expected len ≤ 31, got %d", len(got))
	}
}
...
func TestCenterText_Normal(t *testing.T) {
	...
	if len(got) < 2 || got[len(got)-2:] != "hi" {
		t.Errorf("text not found in result: %q", got)
	}
...
func TestCenterText_ExactWidth(t *testing.T) {
	...
	if got != "hello" {
		t.Errorf("expected unchanged text for exact width, got %q", got)
	}
}
...
func TestParseSelection_ZeroIndex(t *testing.T) {
	...
	if len(got) != 0 {
		t.Errorf("expected empty for zero index, got %v", got)
	}
}
...
func TestParseSelection_RangeOutOfBounds(t *testing.T) {
	...
	if len(got) != 0 {
		t.Errorf("expected empty for range exceeding max, got %v", got)
	}
}
